...
callapinfoActivity = rpc.declare({
	object: 'apinfo',
	method: 'activity',
	params: [ 'section' ],
	expect: { '': {} }
...
callapinfoScripts = rpc.declare({
	object: 'apinfo',
	method: 'scripts',
	expect: { '': {} }
});

callapinfo = rpc.declare({
	object: 'apinfo',
	method: 'status',
	expect: { '': {} }
});
...
function hostStatus(records, section_id) {
	...
	if (lastContact == -1)
		...
		const interval = uci.get('apinfo', '@global[0]', 'interval');
		if (!interval)
...
return view.extend({
	...
	handleSaveApply: function(ev, mode) {
		...
		return this.super('handleSaveApply', [ev, mode]).then(() => {
			...
			return fs.read('/etc/crontabs/root').then((data) => {
				...
				const regLine = /^(\*\/\d+ \* \* \* \* \/usr\/bin\/apinfo)$/m;
				const wanted = `*/${interval} * * * * /usr/bin/apinfo`;
	...
	render: function(data) {
		...
		function refreshDeviceGrid() {
			...
			document.querySelectorAll('#cbi-apinfo-host tr[data-section-id]').forEach(row => {
		...
		s.renderContents = function(/* ... */) {
			...
			return Promise.resolve(renderTask).then(function(nodes) {
				...
				let devicesonline = 0;
				let devicesoffline = 0;
				nodes.querySelectorAll('#cbi-apinfo-host tr[data-section-id]').forEach(row => {
		...
		s.handleAdd = function(ev) {
			...
			const newSid = nextFreeSid('host', uci.sections('apinfo', 'host').length);
			this.map.data.add('apinfo', 'host', newSid);
		...
		s.actionActivity = function(section_id) {
			return L.resolveDefault(callapinfoActivity(section_id), {}).then(function (res) {
		...
		s.actionExec = function(section_id) {
			...
			return callapinfoScripts().then(function(res) {
		...
		s.renderContents = function(/* ... */) {
			...
			return Promise.resolve(renderTask).then(function(nodes) {
				let e = nodes.querySelector('#cbi-apinfo-global > h3')
				if (e)
					e.remove();
				return nodes;
			});
		...
		s.renderContents = function(/* ... */) {
			...
			return Promise.resolve(renderTask).then(function(nodes) {
				...
				let e = nodes.querySelector('#cbi-apinfo-additionalscript > h3');
		...
		poll.add(() => {
			return callapinfo().then(livestatus => {
				updateDeviceStatus(livestatus);
				refreshDeviceGrid();
			});
		}, 63);
